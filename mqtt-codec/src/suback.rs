...
impl PacketCodec<SubAck> for SubAck {
    fn from_buf_extra(buf: &mut BytesMut, fixed_header: FixedHeader) -> Result<SubAck, Error> {
        let variable_header =
            SubAckVariableHeader::from_buf(buf).expect("Failed to parse SubAck Variable Header");
...
impl Frame<SubAck> for SubAck {
    ...
    fn from_buf(buf: &mut BytesMut) -> Result<SubAck, Error> {
        ...
        assert_eq!(
            ...
            "The dup of SubAck Fixed Header must be set to false"
        );
        assert_eq!(
            fixed_header.qos,
            Qos::AtMostOnce,
            "The qos of SubAck Fixed Header must be set to be AtMostOnce"
        );
        assert_eq!(
            fixed_header.retain, false,
            "The retain of SubAck Fixed Header must be set to false"
        );
            ...
            match key {
                ...
                _ => {
                    return Err(Error::InvalidPropertyType(
                        "SubAck Properties contains a invalid property".to_string(),
                    ))
                }
            }
        }
...
impl Frame<SubAckVariableHeader> for SubAckVariableHeader {
    ...
    fn from_buf(buf: &mut BytesMut) -> Result<SubAckVariableHeader, Error> {
        ...
        let mut suback_property =
            Mqtt5Property::from_buf(buf).expect("Failed to parse SubAck Properties");
...
mod test {
    ...
    fn test_suback() {
        ...
        let suback = SubAck::from_buf(&mut buf).expect("Failed to parse SubAck Packet");

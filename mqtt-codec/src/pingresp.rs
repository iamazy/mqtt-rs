...
impl Frame<PingResp> for PingResp {
    ...
    fn from_buf(buf: &mut BytesMut) -> Result<PingResp, Error> {
        ...
        assert_eq!(
            ...
            "The dup of PingResp Fixed Header must be set to false"
        );
        assert_eq!(
            fixed_header.qos,
            Qos::AtMostOnce,
            "The qos of PingResp Fixed Header must be set to be AtMostOnce"
        );
        assert_eq!(
            fixed_header.retain, false,
            "The retain of PingResp Fixed Header must be set to false"
        );
...
mod test {
    ...
    fn test_pingresp() {
        ...
        let pingresp = PingResp::from_buf(&mut buf).expect("Failed to parse PingResp Packet");

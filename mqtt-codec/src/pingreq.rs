...
impl Frame<PingReq> for PingReq {
    ...
    fn from_buf(buf: &mut BytesMut) -> Result<PingReq, Error> {
        ...
        assert_eq!(
            ...
            "The dup of PingReq Fixed Header must be set to false"
        );
        assert_eq!(
            fixed_header.qos,
            Qos::AtMostOnce,
            "The qos of PingReq Fixed Header must be set to be AtMostOnce"
        );
        assert_eq!(
            fixed_header.retain, false,
            "The retain of PingReq Fixed Header must be set to false"
        );
...
mod test {
    ...
    fn test_pingreq() {
        ...
        let pingreq = PingReq::from_buf(&mut buf).expect("Failed to parse PingReq Packet");

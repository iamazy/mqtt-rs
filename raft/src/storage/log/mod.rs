...
pub trait Store: Display + Sync + Send {
    /// Append a log entry, return its index
    fn append(&mut self, entry: Vec<u8>) -> Result<u64>;

    /// Commit log entries up to and including the given index, making them immutable.
    ...
    /// Truncates the log be removing any entries above the give index,
    /// and returns the highest index, Errors if asked to truncate any committed
    /// entries
...
impl Range {
    ...
    /// RangeBound directly in scan() since that prevents us from Store into
    /// a trait object
